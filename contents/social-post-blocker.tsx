		...
		>
			...
			<div className="feed-ly-compact">
				...
				<button
					...
					type="button"
					className="feed-ly-compact-button"
				>
...
async function applyPostCover(
	...
	if (!unmutedPosts.includes(postHash)) {
		...
		if (!document.getElementById("feed-ly-styles")) {
			...
			style.textContent = `
        /* Scoped FeedMax styles with more aggressive specificity */
        .feed-ly-cover {
	...
	const processPost = useCallback(async (container: Element) => {
		...
		try {
			...
			try {
				...
				if (debugObj?.categorizeCache?.has(postHash)) {
					console.log(
	...
	const cssRules = Array.from(document.styleSheets)
		.filter((sheet) => {
			try {
				...
				return (
					sheet.href === null || sheet.href.includes("chrome-extension://")
				);
			} catch (e) {
				return false;
			}
